    ...
    <div class="i_settings_wrapper_items"> 
            ...
            <?php 
               ...
               if($payedActiveSubscriptions){ 
                  foreach($payedActiveSubscriptions as $payedSub){
                    ...
                    $userNetEarningC = $payedSub['user_net_earning'];
                    ...
                    $payedSubscriberUserName = isset($patedUserData['i_username']) ? $patedUserData['i_username'] : NULL;
                    $payedSubscriberUserFullName = isset($patedUserData['i_user_fullname']) ? $patedUserData['i_user_fullname'] : NULL;
                ...
                <div class="i_tab_list_item flex_">
                    ...
                    <div class="tab_detail_item item_mobile">
                        ...
                        if($paymentMethod == 'stripe'){
                            echo filter_var($currencys[$payedCurrency], FILTER_SANITIZE_STRING).$payedAmount;  
                        }else{
                            echo round($payedAmount,2).'<span style="font-size:12px;padding-left:5px;font-style:italic;">'.$LANG['points'].'</span>'; 
                        }?> 
                    </div>
                    <div class="tab_detail_item">
                        <?php 
                        if($paymentMethod == 'stripe'){
                            echo filter_var($currencys[$payedCurrency], FILTER_SANITIZE_STRING).number_format($netEarning, 2);
                        }else{
                            echo filter_var($currencys[$payedCurrency], FILTER_SANITIZE_STRING).number_format($userNetEarningC, 2); 
                        }?>
                    </div>
     ...
     <div class="i_become_creator_box_footer tabing">
        <?php if (ceil($totalSubscribers / $paginationLimit) > 0): ?>
            <ul class="pagination">
                <?php if ($pagep > 1): ?>
                <li class="prev"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)-1 ?>"><?php echo filter_var($LANG['preview_page'], FILTER_SANITIZE_STRING);?></a></li>
                <?php endif; ?>

                <?php if ($pagep > 3): ?>
                <li class="start"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=1">1</a></li>
                <li class="dots">...</li>
                <?php endif; ?>

                <?php if ($pagep-2 > 0): ?><li class="page"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)-2; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING)-2; ?></a></li><?php endif; ?>
                <?php if ($pagep-1 > 0): ?><li class="page"><a href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)-1; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING)-1; ?></a></li><?php endif; ?>

                <li class="currentpage active"><a  class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING); ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING); ?></a></li>

                <?php if ($pagep+1 < ceil($totalSubscribers / $paginationLimit)+1): ?><li class="page"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)+1; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING)+1; ?></a></li><?php endif; ?>
                <?php if ($pagep+2 < ceil($totalSubscribers / $paginationLimit)+1): ?><li class="page"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)+2; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING)+2; ?></a></li><?php endif; ?>

                <?php if ($pagep < ceil($totalSubscribers / $paginationLimit)-2): ?>
                <li class="dots">...</li>
                <li class="end"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo ceil($totalSubscribers / $paginationLimit); ?>"><?php echo ceil($totalSubscribers / $paginationLimit); ?></a></li>
                <?php endif; ?>

                <?php if ($pagep < ceil($totalSubscribers / $paginationLimit)): ?>
                <li class="next"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING);?>settings?tab=subscription_payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING)+1; ?>"><?php echo filter_var($LANG['next_page'], FILTER_SANITIZE_STRING);?></a></li>
                <?php endif; ?>
            </ul>
        <?php endif; ?>    
     </div> 
