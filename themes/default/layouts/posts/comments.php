            ...
            <div class="i_comment_call_popup openComMenu" id="<?php echo filter_var($commentID, FILTER_SANITIZE_STRING);?>">
                ...
                <div class="i_comment_menu_container comMBox comMBox<?php echo filter_var($commentID, FILTER_SANITIZE_STRING);?>">
                    <div class="i_comment_menu_wrapper">
                        ...
                        $reportText = $LANG['report_comment'];
                        if($checkCommentReportedBeforeByUserID == '1'){
                          $reportText = $LANG['unreport'];
                        }
                        ?>
                        <!--MENU ITEM-->
                        <div class="i_post_menu_item_out ccp transition ccp<?php echo filter_var($commentID, FILTER_SANITIZE_STRING);?>" id="<?php echo filter_var($commentID, FILTER_SANITIZE_STRING);?>" data-id="<?php echo filter_var($userPostID, FILTER_SANITIZE_STRING);?>">
                        <?php echo html_entity_decode($iN->iN_SelectedMenuIcon('32'));?> <?php echo filter_var($reportText, FILTER_SANITIZE_STRING);?>
                        </div> 
                        <!--/MENU ITEM-->
                        <?php }?>
                    </div>
